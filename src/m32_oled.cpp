...
#if POCKETSYNTH_ENABLE_M32_OLED
#define M32_OLED_MAYBE_UNUSED
#else
#define M32_OLED_MAYBE_UNUSED [[maybe_unused]]
#endif
...
enum class M32OledFeedbackType : uint8_t {
  Note,
  ControlChange,
  PitchBend,
};
...
M32_OLED_MAYBE_UNUSED void publishStarted() {
...
M32_OLED_MAYBE_UNUSED void publishClientRegistered(esp_err_t err) {
  portENTER_CRITICAL(&gM32OledMux);
...
M32_OLED_MAYBE_UNUSED void publishQueueState(bool hasQueuedFrame) {
  portENTER_CRITICAL(&gM32OledMux);
  gStatus.frameQueued = hasQueuedFrame;
  portEXIT_CRITICAL(&gM32OledMux);
}

void publishFeedback(const char* feedbackName) {
  portENTER_CRITICAL(&gM32OledMux);
  ++gStatus.feedbackCount;
  copyString(gStatus.lastFeedback, sizeof(gStatus.lastFeedback), feedbackName);
  portEXIT_CRITICAL(&gM32OledMux);
}

const char* noteName(uint8_t midi) {
  switch (midi % 12U) {
    case 0: return "C";
    case 1: return "C#";
    case 2: return "D";
...
void writeNoteLabel(char* out, size_t outSize, uint8_t midi) {
  if (out == nullptr || outSize == 0) {
    return;
  }
  const int octave = static_cast<int>(midi / 12U) - 1;
  snprintf(out, outSize, "%s%d", noteName(midi), octave);
}

const char* controlLabel(uint8_t control) {
  switch (control) {
    case 1: return "MOD STRIP";
    case 14: return "KNOB 1";
    case 15: return "KNOB 2";
    case 16: return "KNOB 3";
    case 17: return "KNOB 4";
    case 18: return "KNOB 5";
    case 19: return "KNOB 6";
    case 20: return "KNOB 7";
    case 21: return "KNOB 8";
...
M32_OLED_MAYBE_UNUSED void drawFeedbackEvent(const M32OledFeedbackEvent& event) {
  ...
  const char* feedbackName = "event";

  switch (event.type) {
    case M32OledFeedbackType::Note: {
      char note[8] = {};
      writeNoteLabel(note, sizeof(note), event.first);
      snprintf(line, sizeof(line), "%s %s", event.pressed ? "NOTE ON" : "NOTE OFF", note);
      snprintf(value, sizeof(value), "VEL %u", static_cast<unsigned int>(event.second));
      normalized = static_cast<float>(event.second) / 127.0f;
      feedbackName = event.pressed ? "note_on" : "note_off";
      break;
    }
    case M32OledFeedbackType::ControlChange: {
      const char* label = controlLabel(event.first);
      if (label != nullptr) {
        snprintf(line, sizeof(line), "%s", label);
      } else {
        snprintf(line, sizeof(line), "CC %03u", static_cast<unsigned int>(event.first));
      }
      snprintf(value, sizeof(value), "VAL %u", static_cast<unsigned int>(event.second));
      normalized = static_cast<float>(event.second) / 127.0f;
      feedbackName = "control";
      break;
    }
    case M32OledFeedbackType::PitchBend: {
      snprintf(line, sizeof(line), "PITCH BEND");
      snprintf(value, sizeof(value), "%+d", static_cast<int>(event.signedValue));
      normalized = (static_cast<float>(event.signedValue) + 8192.0f) / 16383.0f;
...
void initializeStatusDefaults() {
  ...
  copyString(gStatus.lastFeedback, sizeof(gStatus.lastFeedback), "none");
  portEXIT_CRITICAL(&gM32OledMux);
}
...
M32_OLED_MAYBE_UNUSED void buildOledRegionReport(uint8_t* out, const M32OledQueuedFrame& frame, uint16_t page) {
...
void buildOledRegionReportFromBits(uint8_t* out, const uint8_t* bits, uint16_t page) {
  if (out == nullptr || bits == nullptr) {
    return;
  }
  ...
  for (int localPage = 0; localPage < static_cast<int>(M32_OLED_REGION_HEIGHT_PAGES); ++localPage) {
    for (int x = 0; x < M32_OLED_WIDTH; ++x) {
      ...
      for (int bit = 0; bit < 8; ++bit) {
        const int y = static_cast<int>((page + localPage) * 8U) + bit;
        const size_t srcIndex = static_cast<size_t>(y * M32_OLED_WIDTH + x);
        const uint8_t srcByte = bits[srcIndex / 8U];
        const bool on = (srcByte & (0x80U >> (srcIndex & 7U))) != 0;
        if (on) {
          packed |= static_cast<uint8_t>(1U << bit);
        }
...
void appendStatusJson(JsonWriter* writer, const M32OledStatus& status) {
  appendFormat(writer,
               ...
               static_cast<unsigned long>(status.feedbackCount),
               static_cast<unsigned long>(status.errorCount));
  appendJsonString(writer, status.lastEvent);
  appendFormat(writer, ",\"last_feedback\":");
  appendJsonString(writer, status.lastFeedback);
  appendFormat(writer,
               ",\"last_event_ms\":%lu,"
               "\"last_frame_ms\":%lu,"
               "\"last_error\":\"%s\","
...
[[maybe_unused]] void m32OledHostTask(void*) {
...
void m32OledFeedbackTask(void*) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(gFeedbackQueue, &event, portMAX_DELAY) != pdTRUE) {
      continue;
    }

    M32OledFeedbackEvent latest = event;
...
esp_err_t initializeM32Oled() {
  ...
  publishStarted();
  portENTER_CRITICAL(&gM32OledMux);
  gStatus.clientRegistered = true;
  setLastEventUnlocked(&gStatus, "service_ready", ESP_OK);
  portEXIT_CRITICAL(&gM32OledMux);
  ...
  if (gFeedbackQueue == nullptr) {
    ...
    if (gFeedbackQueue == nullptr) {
      publishError("feedback_queue_error", ESP_ERR_NO_MEM);
      return ESP_ERR_NO_MEM;
    }
  }

  addDiagnosticLog("I", TAG, "M32 OLED framebuffer service ready");
  const BaseType_t feedbackCreated = xTaskCreatePinnedToCore(m32OledFeedbackTask,
                                                             "M32OledFeedback",
                                                             M32_OLED_FEEDBACK_TASK_STACK,
                                                             nullptr,
                                                             M32_OLED_FEEDBACK_TASK_PRIORITY,
                                                             nullptr,
                                                             0);
  if (feedbackCreated != pdTRUE) {
...
void notifyM32OledMidiNote(uint8_t midi, bool pressed, uint8_t velocity) {
  ...
  M32OledFeedbackEvent event = {};
  event.type = M32OledFeedbackType::Note;
  event.first = midi;
  event.second = velocity;
...
void notifyM32OledControlChange(uint8_t control, uint8_t value) {
  ...
  M32OledFeedbackEvent event = {};
  event.type = M32OledFeedbackType::ControlChange;
  event.first = control;
  event.second = value;
  BaseType_t sent = xQueueSend(gFeedbackQueue, &event, 0);
  if (sent != pdTRUE) {
...
void notifyM32OledPitchBend(int16_t pitchBend) {
  ...
  M32OledFeedbackEvent event = {};
  event.type = M32OledFeedbackType::PitchBend;
  event.signedValue = pitchBend;
  BaseType_t sent = xQueueSend(gFeedbackQueue, &event, 0);
  if (sent != pdTRUE) {
    M32OledFeedbackEvent dropped = {};
    (void)xQueueReceive(gFeedbackQueue, &dropped, 0);
...
bool takeNextM32OledFrame(M32OledFramebuffer* frame) {
  ...
  M32OledQueuedFrame queued = {};
  if (xQueueReceive(gFrameQueue, &queued, 0) != pdTRUE) {
    publishQueueState(false);
    return false;
  }
  memcpy(frame->data(), queued.bits, sizeof(queued.bits));
  publishQueueState(uxQueueMessagesWaiting(gFrameQueue) > 0);
  return true;
#else
  (void)frame;
...
esp_err_t writeM32OledRegionReport(uint8_t* out,
                                   ...
                                   uint16_t page) {
  if (out == nullptr || outSize < M32_OLED_REPORT_BYTES ||
      (page != 0 && page != M32_OLED_REGION_HEIGHT_PAGES)) {
    return ESP_ERR_INVALID_ARG;
  }
  buildOledRegionReportFromBits(out, frame.data(), page);
  return ESP_OK;
}

void publishM32OledTransportConnected(uint8_t address,
                                      uint8_t interfaceNumber,
                                      uint8_t alternateSetting,
                                      uint8_t endpointAddress,
                                      uint16_t endpointMaxPacketSize,
  ...
  portENTER_CRITICAL(&gM32OledMux);
  gStatus.deviceConnected = true;
  gStatus.interfaceClaimed = interfaceClaimed;
  gStatus.address = address;
  gStatus.interfaceNumber = interfaceNumber;
  gStatus.alternateSetting = alternateSetting;
  gStatus.endpointAddress = endpointAddress;
  gStatus.endpointMaxPacketSize = endpointMaxPacketSize;
  gStatus.vid = vid;
  gStatus.pid = pid;
  ++gStatus.connectCount;
  setLastEventUnlocked(&gStatus, "connected", ESP_OK);
...
void publishM32OledTransportDisconnected(uint8_t address) {
  portENTER_CRITICAL(&gM32OledMux);
  gStatus.deviceConnected = false;
  gStatus.interfaceClaimed = false;
  gStatus.transferActive = false;
...
void publishM32OledTransportActive(bool active, esp_err_t err) {
  portENTER_CRITICAL(&gM32OledMux);
  gStatus.transferActive = active;
  if (active) {
    ++gStatus.transferCount;
...
void publishM32OledTransportError(const char* eventName, esp_err_t err) {
  publishError(eventName, err);
}

void publishM32OledTransportFrameSent() {
#if POCKETSYNTH_ENABLE_M32_OLED
  publishFrameSent();
#endif
}
